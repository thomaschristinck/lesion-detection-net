...
void rleNms( RLE *dt, siz n, uint *keep, double thr ) {
  ...
  for( i=0; i<n; i++ ) if(keep[i]) {
    for( j=i+1; j<n; j++ ) if(keep[j]) {
      rleIou(dt+i,dt+j,1,1,&u);
      if(u>thr) keep[j]=0;
    }
  }
}

void bbIou( BB dt, BB gt, siz m, siz n, double *o ) {
  double h, w, i, u, ga, da; siz g, d;
  for( g=0; g<n; g++ ) {
    BB G=gt+g*4; ga=G[2]*G[3];
    for( d=0; d<m; d++ ) {
